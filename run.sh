...
docker stop gensyn-telegram-bot
docker rm gensyn-telegram-bot
docker run -dit --restart always --name gensyn-telegram-bot cats/gensyn-exporter python ./check_alive.py
